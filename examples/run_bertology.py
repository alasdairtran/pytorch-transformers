...
def compute_heads_importance(args, model, eval_dataloader, compute_entropy=True, compute_importance=True, head_mask=None):
    ...
    for step, batch in enumerate(tqdm(eval_dataloader, desc="Iteration", disable=args.local_rank not in [-1, 0])):
        ...
        outputs = model(input_ids, token_type_ids=segment_ids,
                        attention_mask=input_mask, labels=label_ids, head_mask=head_mask)
...
def mask_heads(args, model, eval_dataloader):
    ...
    original_score = compute_metrics(args.task_name, preds, labels)[
        args.metric_name]
    ...
    while current_score >= original_score * args.masking_threshold:
        ...
        preds = np.argmax(
            preds, axis=1) if args.output_mode == "classification" else np.squeeze(preds)
        current_score = compute_metrics(args.task_name, preds, labels)[
            args.metric_name]
...
def prune_heads(args, model, eval_dataloader, head_mask):
    ...
    preds = np.argmax(
        preds, axis=1) if args.output_mode == "classification" else np.squeeze(preds)
    score_masking = compute_metrics(args.task_name, preds, labels)[
        args.metric_name]
    ...
    preds = np.argmax(
        preds, axis=1) if args.output_mode == "classification" else np.squeeze(preds)
    score_pruning = compute_metrics(args.task_name, preds, labels)[
        args.metric_name]
...
def main():
    ...
    model = model_class.from_pretrained(args.model_name, from_tf=bool(
