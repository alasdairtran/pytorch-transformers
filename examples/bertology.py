...
def compute_heads_importance(args, model, eval_dataloader, compute_entropy=True, compute_importance=True, head_mask=None):
    ...
    for step, batch in enumerate(tqdm(eval_dataloader, desc="Iteration", disable=args.local_rank not in [-1, 0])):
        ...
        all_attentions, logits = model(
            input_ids, token_type_ids=segment_ids, attention_mask=input_mask, head_mask=head_mask)

        if compute_entropy:
            # Update head attention entropy
            for layer, attn in enumerate(all_attentions):
                masked_entropy = entropy(
                    attn.detach()) * input_mask.float().unsqueeze(1)
        ...
        if compute_importance:
            ...
            for layer, mh_layer_output in enumerate(multihead_outputs):
                dot = torch.einsum(
                    "bhli,bhli->bhl", [mh_layer_output.grad, mh_layer_output])
...
def run_model():
    ...
    parser.add_argument('--model_name_or_path', type=str, default='bert-base-cased-finetuned-mrpc',
    ...
    parser.add_argument("--dont_normalize_importance_by_layer",
                        action='store_true', help="Don't normalize importance score by layers")
    ...
    try:
        ...
        eval_features = convert_examples_to_features(
            eval_examples, label_list, args.max_seq_length, tokenizer, args.output_mode)
    ...
    all_label_ids = torch.tensor([f.label_id for f in eval_features],
                                 dtype=torch.long if args.output_mode == "classification" else torch.float)
    eval_data = TensorDataset(
        all_input_ids, all_input_mask, all_segment_ids, all_label_ids)

    if args.data_subset > 0:
        eval_data = Subset(eval_data, list(
            range(min(args.data_subset, len(eval_data)))))
    ...
    head_ranks[head_importance.view(-1).sort(descending=True)[1]
    ...
    if args.try_masking and args.masking_threshold > 0.0 and args.masking_threshold < 1.0:
        ...
        original_score = compute_metrics(task_name, preds, labels)[
            args.metric_name]
        ...
        while current_score >= original_score * args.masking_threshold:
            ...
            logger.info("Heads to mask: %s", str(
                current_heads_to_mask.tolist()))
            ...
            preds = np.argmax(
                preds, axis=1) if args.output_mode == "classification" else np.squeeze(preds)
            current_score = compute_metrics(task_name, preds, labels)[
                args.metric_name]
            logger.info("Masking: current score: %f, remaning heads %d (%.1f percents)",
                        current_score, new_head_mask.sum(), new_head_mask.sum()/new_head_mask.numel() * 100)
        ...
        preds = np.argmax(
            preds, axis=1) if args.output_mode == "classification" else np.squeeze(preds)
        score_masking = compute_metrics(task_name, preds, labels)[
            args.metric_name]
        ...
        preds = np.argmax(
            preds, axis=1) if args.output_mode == "classification" else np.squeeze(preds)
        score_pruning = compute_metrics(task_name, preds, labels)[
            args.metric_name]
        new_time = datetime.now() - before_time

        logger.info("Pruning: original num of params: %.2e, after pruning %.2e (%.1f percents)",
