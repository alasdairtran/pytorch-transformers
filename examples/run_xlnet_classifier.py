...
def main():
    ...
    tokenizer = XLNetTokenizer.from_pretrained(
        args.xlnet_model, do_lower_case=args.do_lower_case)
    model = XLNetForSequenceClassification.from_pretrained(
        args.xlnet_model, num_labels=num_labels)
    ...
    if args.do_train:
        ...
        if os.path.exists(cached_train_features_file):
            logger.info("Loading train features for cache file %s",
                        cached_train_features_file)
            with open(cached_train_features_file, "rb") as reader:
                train_features = pickle.load(reader)
        else:
            logger.info("No cache file at %s, preparing train features",
                        cached_train_features_file)
        ...
        if args.local_rank == -1:
            train_sampler = SequentialSampler(
                train_data)  # RandomSampler(train_data)
        else:
        ...
        for _ in trange(int(args.num_train_epochs) if args.max_steps <= 0 else int('Inf'),
            ...
            for step, batch in enumerate(tqdm(train_dataloader,
                ...
                loss, _ = model(input_ids, token_type_ids=segment_ids,
                ...
                gnorm = torch.nn.utils.clip_grad_norm_(
                    model.parameters(), args.clip_gradients)
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    if args.local_rank in [-1, 0] and (args.log_every <= 0 or (global_step + 1) % args.log_every == 0):
                        ...
                        )[0] if not args.fp16 else lr_this_step
                        ...
                        tb_writer.add_scalar(
                            'loss', curr_tr_loss / curr_steps, global_step)
                        curr_tr_loss, curr_steps = 0., 1
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        if os.path.exists(cached_eval_features_file):
            logger.info("Loading eval features for cache file %s",
                        cached_eval_features_file)
            with open(cached_eval_features_file, "rb") as reader:
                eval_features = pickle.load(reader)
        else:
            logger.info("No cache file at %s, preparing eval features",
                        cached_eval_features_file)
        ...
        if task_name == "mnli":
            ...
            for input_ids, input_mask, segment_ids, label_ids in tqdm(eval_dataloader, desc="Evaluating"):
                ...
                with torch.no_grad():
                    logits, _ = model(
                        input_ids, token_type_ids=segment_ids, attention_mask=input_mask, labels=None)
