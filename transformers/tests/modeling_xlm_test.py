...
import pytest
from pytorch_pretrained_bert import (XLMConfig, XLMForQuestionAnswering,
    ...
    class XLMModelTester(object):
        ...
        def prepare_config_and_inputs(self):
            ...
            input_mask = ids_tensor(
                [self.batch_size, self.seq_length], 2).float()

            input_lengths = None
            if self.use_input_lengths:
                # small variation of seq_length
                input_lengths = ids_tensor(
                    [self.batch_size], vocab_size=2) + self.seq_length - 2
        ...
        def create_and_check_xlm_model(self, config, input_ids, token_type_ids, input_lengths, sequence_labels, token_labels, is_impossible_labels, input_mask):
            ...
            model.eval()
            outputs = model(input_ids, lengths=input_lengths,
